...
> Projeto em desenvolvimento inicial. Autenticação, validações e regras de negócio mais avançadas ainda serão adicionadas.
...
2. **Configure a connection string** em `src/API/appsettings.json`:
   ```json
   "ConnectionStrings": {
...
3. **Aplique as migrations** (cria as tabelas no banco):
   ```bash
